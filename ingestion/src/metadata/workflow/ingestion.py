...
class IngestionWorkflow(BaseWorkflow, ABC):
    ...
    def __init__(self, config: OpenMetadataWorkflowConfig):
        ...
        logger.error(f"ingestion.py - INIT - metadata_config: {metadata_config}")
        logger.error(f"ingestion.py - INIT - service_type: {self.service_type}")
        logger.error(f"ingestion.py - INIT - config: {config}")
    ...
    def post_init(self) -> None:
        ...
        logger.error(f"ingestion.py - POST INIT")
...
class ImportIngestionWorkflow(BaseWorkflow, ABC):
    """
    Base Ingestion Workflow implementation
    """
    ...
    def __init__(self, config: OpenMetadataWorkflowConfig, ingestion_pipline: IngestionPipeline):
        self.config = config
        # config: OpenMetadataWorkflowConfig(source=Source(type='postgres', serviceName='TestPostgres46', serviceConnection=ServiceConnection(ro...chReindexingExtraction(root=True), extraHeaders=None), config=None), ingestionPipelineFQN='TestPostgres46', pipelineRunId=None)
        self.service_type: ServiceType = get_service_type_from_source_type(
            self.config.source.type
        )
        ...
        # metadata_config: OpenMetadataConnection = OpenMetadataConnection(clusterName='openmetadata', type=<OpenmetadataType.OpenMetadata: 'OpenMetadata'>, hostPort='http://192....(root=True), supportsElasticSearchReindexingExtraction=SupportsElasticSearchReindexingExtraction(root=True), extraHeaders=None)
        ...
        super().__init__(
            ...
            metadata_config=metadata_config,
            service_type=self.service_type,
            _ingestion_pipeline=ingestion_pipline,
        )
        logger.error(f"ingestion.py - INIT - metadata_config: {metadata_config}")
        logger.error(f"ingestion.py - INIT - service_type: {self.service_type}")
        logger.error(f"ingestion.py - INIT - config: {config}")
    ...
    @classmethod
    def importing(cls, config_dict: dict, ingestion_pipline: IngestionPipeline):
        config = parse_workflow_config_gracefully(config_dict)
        return cls(config, ingestion_pipline)
